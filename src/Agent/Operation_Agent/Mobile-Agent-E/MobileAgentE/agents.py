...
def add_final_response(role, prompt, image=None):
    new_chat_history = []
    if image:
...
class Answerer:
    """
    An AI assistant class to answer user questions based on provided text,
    a summary thought, and a screenshot image. It generates both an answer
    and an explanation, sourcing information strictly from the inputs and
    outputting in JSON format.
    """  # Docstring updated slightly

    def init_chat(self) -> list:
        """
        Initializes the chat history with the system prompt.
        The prompt instructs the model to respond ONLY in JSON format.
        """
        chat_history = []
        # ** NEW English System Prompt demanding JSON output **
        system_prompt = """You are an AI assistant specialized in analyzing phone screenshots, user requests, and summary thoughts ('finish thought'). Your goal is to answer the user's question accurately, providing both the answer and a clear explanation based *exclusively* on the provided information.

**--- Task: Analyze inputs and generate the specified JSON object ---**
Carefully analyze the screenshot, the user's specific request, and the summarized 'Finish thought' to populate the JSON response according to these strict rules.

**--- Strict output specification (MUST FOLLOW) ---**
1. **JSON format only**:
- Must output a pure JSON object without any additional packaging
- Do not use ```json and other Markdown symbols
- Do not include comments/description text

2. **Your response format will look like this**:
{
    "answer": "Direct answer synthesized using only input information",
    "description": "Three-paragraph explanation based only on input information: 1) Screenshot analysis 2) User needs analysis 3) Conclusion derivation"
}

3. **Data source restrictions**:
- When there is insufficient information, answer is set to "Insufficient information cannot be answered"
- Any external knowledge supplement is prohibited (even if you think the information should exist)

**--- Validation Examples (VALIDATION EXAMPLES) ---**
◆ Correct example:
{
    "answer": "The current screen shows 15% remaining power",
    "description": "1) The battery icon in the status bar at the top of the screenshot is red and shows 15% | 2) The user asks about the power status | 3) Comprehensively determine that the current power is low"
}

◆ Incorrect example:
```json // Prohibit code block identification
{
    answer: 'Unauthorized access', // Prohibit single quotes
    "description": "According to industry standards..." // Prohibit external knowledge
}"""

        # Using the expected format for add_response/API
        chat_history.append(["system", [{"type": "text", "text": system_prompt}]])
        return chat_history

    def _parse_model_output(self, raw_output: str) -> dict:
        """
        Parses the raw output string from the AI model, expecting it to be
        a valid JSON string conforming to the system prompt's template.

        Args:
            raw_output (str): The raw text output from the AI model, expected
                              to be a JSON string.

        Returns:
            dict: A dictionary containing the 'answer' and 'description',
                  extracted from the JSON, or error messages if parsing fails
                  or the format is incorrect.
        """
        try:
            # Attempt to parse the entire raw output as JSON
            data = json.loads(raw_output)

            # Validate if it's a dictionary and contains the expected keys
            if isinstance(data, dict):
                answer = data.get("answer", "Error: JSON missing 'answer' key.")
                description = data.get(
                    "description", "Error: JSON missing 'description' key."
                )
                return {"answer": answer, "description": description}
            else:
                # The string was valid JSON, but not the expected object type
                return {
                    "answer": "Error: Model returned valid JSON, but not the expected object format.",
                    "description": f"Received type: {type(data)}, Raw: {raw_output}",
                }

        except json.JSONDecodeError:
            # The model's output was not valid JSON
            print(f"--- JSON PARSING FAILED ---")  # Debugging info
            print(f"Raw output was:\n{raw_output}")
            print(f"--- END RAW OUTPUT ---")
            return {
                "answer": "Error: Model did not return a valid JSON string.",
                "description": f"Invalid JSON received: {raw_output}",
            }
        except Exception as e:
            ...
            print(f"--- END RAW OUTPUT ---")
            return {
                "answer": f"Error: An unexpected error occurred during parsing: {e}",
                "description": f"Raw output: {raw_output}",
            }

    # def process_and_answer(
    #     self, instruction: str, finish_thought: str, image=None, temperature: float = 0
    # ) -> dict:
    #     """
    #     Processes the user's request, finish thought, and screenshot to generate
    #     a JSON response containing an answer and explanation using the AI model.

    #     Args:
    #         instruction (str): The user's specific question or request.
    #         finish_thought (str): A summary thought related to the context/screenshot.
    #         screenshot_file (str): Path to the screenshot image file.
    #         temperature (float): Temperature setting for the AI model's response generation.

    #     Returns:
    #         dict: A dictionary containing the 'answer' and 'description' (explanation),
    #               parsed from the model's JSON response, or error messages if parsing fails.
    #     """
    #     chat_history = self._init_chat()

    #     # Construct the final prompt for the user turn
    #     # The core request remains, but the AI's output format is dictated by the system prompt
    #     final_prompt_text = "User's question:\n"
    #     final_prompt_text += instruction + "\n\n"
    #     final_prompt_text += "Finish thought:\n"
    #     final_prompt_text += finish_thought + "\n\n"
    #     final_prompt_text += "Based *only* on the information above and the provided image, generate the required JSON response."  # Updated final instruction slightly

    #     # Add the user's request and image to the chat history
    #     chat_history = add_response(
    #         "user", final_prompt_text, chat_history, image=image
    #     )

    #     # Call the AI model API
    #     raw_output = get_answer_model_api_response(
    #         chat_history, temperature=temperature
    #     )

    #     print(
    #         f"Raw Model Output (expected JSON):\n{raw_output}\n--------------------"
    #     )  # Debugging print

    #     # Parse the raw output, now expecting JSON
    #     parsed_output = self._parse_model_output(raw_output)

    #     return parsed_output

    def save_output(
        self, log_root: str, atomic_tasks_numbers: str, answer: str, description: str
    ):
        """
        Saves the generated answer and description to a JSON file in the specified directory.
        (This method remains unchanged as it already saves in JSON format and expects
         'answer' and 'description'.)

        Args:
            log_root (str): The root directory where logs/outputs are saved.
            atomic_tasks_numbers (str): An identifier (presumably numeric) for the specific task.
            answer (str): The answer extracted from the model's JSON response.
            description (str): The explanation extracted from the model's JSON response.
        """
        output_dir = os.path.join(log_root, str(atomic_tasks_numbers))
        output_file = os.path.join(output_dir, "task_answer.json")
        ...
        output_data = {
            "atomic_tasks_ID": atomic_tasks_numbers,
            "answer": answer,
            "description": description,  # Key name matches the expected JSON and internal use
        }
...
#     # 1. Instantiate the Answerer
#     answerer = Answerer()

#     # 2. Define inputs
#     user_instruction = "Read the mobile device interface and tell me which album Jay Chou released in 2000?"
#     thought_summary = "The task is to identify which album Jay Chou released in 2000. According to the progress made so far, Wikipedia has been consulted and it clearly states that Jay Chou's debut studio album *Jay* was released in 2000. This information directly answers the user\u2019s question. There doesn\u2019t appear to be any ambiguity or need for further investigation since the source (Wikipedia) is reliable and the details provided are clear and relevant."
#     image_path = "src/Agent/Operation_Agent/Mobile-Agent-E/logs/250411/mobile_agent_E/test/20250412-230711/screenshots/3.jpg"
#     task_id = "task_12345"
#     log_directory = "./output_logs"

#     # 3. Call the integrated processing method
#     result = answerer.process_and_answer(
#         instruction=user_instruction,
#         finish_thought=thought_summary,
#         screenshot_file=image_path,
#         temperature=0,
#     )

#     # 4. Print the result dictionary (parsed from JSON)
#     print("Processed Result (Parsed from JSON):")
#     print(
#         result
#     )  # Should now contain keys 'answer' and 'description' or error messages

#     # 5. Save the output (if parsing was successful)
#     if result and "Error:" not in result.get(
#         "answer", ""
#     ):  # Basic check for errors before saving
#         answerer.save_output(
#             log_root=log_directory,
#             atomic_tasks_numbers=task_id,
#             answer=result["answer"],
#             description=result["description"],
#         )
#     else:
#         print(
#             f"Could not save output due to parsing errors or missing keys. Result was: {result}"
#         )
