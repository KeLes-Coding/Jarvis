...
from MobileAgentE.agents import add_response, add_response_two_image, add_final_response
...
BACKBONE_TYPE = os.environ.get(
    "BACKBONE_TYPE", default="OpenAI"
...
OPENAI_ANSWER_API_URL = (
    "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions"
)
OPENAI_ANSWER_API_KEY = ""
...
GEMINI_ANSWER_API_URL = "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions"  # OpenAI compatible
GEMINI_ANSWER_API_KEY = os.environ.get("GEMINI_API_KEY", default=None)

CLAUDE_ANSWER_API_URL = "https://api.anthropic.com/v1/messages"
CLAUDE_ANSWER_API_KEY = os.environ.get("CLAUDE_API_KEY", default=None)
...
if BACKBONE_TYPE == "OpenAI":
    ...
    REASONING_MODEL = "qwen-max-2025-01-25"
    KNOWLEDGE_REFLECTION_MODEL = "qwen-max-2025-01-25"
    ANSWER_MODEL = "qwen-max-2025-01-25"
    # REASONING_MODEL = "llama3.2-vision:90b"
    # KNOWLEDGE_REFLECTION_MODEL = "llama3.2-vision:90b"
    # REASONING_MODEL = "gemma3:12b"
    # KNOWLEDGE_REFLECTION_MODEL = "gemma3:12b"
...
CAPTION_MODEL = "qwen-max-2025-01-25"
...
# OPENAI_API_URL = "http://218.192.110.198:11434/v1/chat/completions"
# OPENAI_API_KEY = "ollama"
# CAPTION_MODEL = "llama3.2-vision:90b"
# CAPTION_MODEL = "gemma3:12b"
...
def generate_api(images, query, caption_model=CAPTION_MODEL):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(process_image, image, query, caption_model=caption_model): i
...
def merge_text_blocks(
    ...
    for i in range(num_blocks):
        ...
        for j in range(i + 1, num_blocks):
            ...
            x_diff_left = abs(
                sorted_coordinates_list[anchor][0] - sorted_coordinates_list[j][0]
            )
            x_diff_right = abs(
                sorted_coordinates_list[anchor][2] - sorted_coordinates_list[j][2]
            )

            y_diff = sorted_coordinates_list[j][1] - sorted_coordinates_list[anchor][3]
            height_anchor = (
                sorted_coordinates_list[anchor][3] - sorted_coordinates_list[anchor][1]
            )
...
def load_perception_models(
    ...
    if caption_call_method == "local":
        if caption_model == "qwen-vl-chat":
            ...
            vlm_model.generation_config = GenerationConfig.from_pretrained(
...
class Perceptor:
    def __init__(self, adb_path, perception_args=DEFAULT_PERCEPTION_ARGS):
        (
            ...
            self.vlm_model,
            self.vlm_tokenizer,
        ) = load_perception_models(**perception_args)
    ...
    def get_perception_infos(self, screenshot_file, temp_file=TEMP_DIR):
        ...
        text, coordinates = ocr(
            screenshot_file, self.ocr_detection, self.ocr_recognition
        )
            ...
            if CAPTION_CALL_METHOD == "local":
                for i in range(len(images)):
                    ...
                    if (
                        ...
                        des = generate_local(
                            self.vlm_tokenizer, self.vlm_model, image_path, prompt
                        )
                    icon_map[i + 1] = des
            else:
                for i in range(len(images)):
...
def get_reasoning_model_api_response(
    chat, model_type=BACKBONE_TYPE, model=None, temperature=0.0
):
...
def get_answer_model_api_response(
    chat, model_type=BACKBONE_TYPE, model=None, temperature=0.0
):

    # chat messages in openai format
    model = ANSWER_MODEL if model is None else model
    if model_type == "OpenAI":
        return inference_chat(
            chat,
            model,
        ...
        return inference_chat(
            chat,
            model,
            CLAUDE_ANSWER_API_URL,
            CLAUDE_ANSWER_API_KEY,
            usage_tracking_jsonl=USAGE_TRACKING_JSONL,
            temperature=temperature,
        )
    else:
...
def run_single_task(
    ...
    temperature=0.0,
    screenrecord=False,
    atomic_tasks_numbers=1,
):

    # log_root = f"logs/{run_name}/{atomic_tasks_numbers}/mobile_agent_E"
    ### set up log dir ###
    if task_id is None:
    ...
    if os.path.exists(log_dir) and not overwrite_log_dir:
        print(
            "The log dir already exists. And overwrite_log_dir is set to False. Skipping..."
        )
        return
    ...
    while True:
        ...
        if iter == 1:  # first perception
            ...
            perception_infos, width, height = perceptor.get_perception_infos(
                screenshot_file, temp_file=TEMP_DIR
            )
            shutil.rmtree(TEMP_DIR)
            os.mkdir(TEMP_DIR)
        ...
        chat_planning = add_response(
            "user", prompt_planning, chat_planning, image=screenshot_file
        )
        ...
        if "Finished" in info_pool.current_subgoal.strip():
            ...
            chat_final = answer.init_chat()
            final_promt = "User's question:\n"
            final_promt += instruction + "\n"
            final_promt += "Finish thought:\n"
            final_promt += info_pool.finish_thought + "\n"
            final_promt += """
**JSON format only**:
...
**Your response format are as follows**:
{
    ...
    "description": "Three-paragraph explanation based only on input information: 1) Screenshot analysis 2) User needs analysis 3) Conclusion derivation"
            ...
            chat_final = add_response(
                "user", final_promt, chat_final, image=screenshot_file
            )
            output_final = get_answer_model_api_response(
                chat_final, temperature=temperature
            )
            print("--- Final Output: ---\n", output_final)
            output_final = answer._parse_model_output(output_final)
            answer.save_output(
                log_root,
                atomic_tasks_numbers,
                output_final["answer"],
                output_final["description"],
            )
